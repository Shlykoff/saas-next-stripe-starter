...
- **Member roles**: `/dashboard/members` shows the roster and lets an owner invite/revoke, but there's no UI yet to change an existing member's role or remove them outright (the RLS policies for it already exist — `organization_members_update_owner` / `organization_members_delete_owner_or_self` — this is a UI gap, not a schema one).
