...
- **Notes**: список пагинируется, ищется, сортируется и обновляется live между участниками (см. пункт про Notes выше); по-прежнему нет вложений. Хватает для демонстрации paywall-гейтинга и реалистичного UX списка; для реального продукта отдельная итерация нужна как минимум под вложения.
