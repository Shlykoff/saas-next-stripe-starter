...
- **Вложения к заметкам** — загрузка файлов в notes. Всё остальное (пагинация, поиск, сортировка, live-обновления — см. пункт про Notes выше) уже сделано; вложения вынесены в отдельную, более крупную фичу (storage, UX загрузки, RLS на файлы), а не влиты в этот раунд.
